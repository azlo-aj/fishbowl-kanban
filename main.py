...
class WOquery():
    ...
    def extract_cstmfld(self, findstr, outputcol, df=None, readstr=r"[A-Z][a-z]*", uppercase=True):
        '''
    ...
    def find_ticket_type(self, df=None):
        '''
        Determines the type of ticket the row item belongs to.
        i.e. "WIP part" or "Assembly"
        '''
        if df is None:
            df=self.df
        for i in df.index:
    ...
    def filter(self, typeid=None, category=None, processed=None, ticket=None, pn=None, wonum=None, df=None):
        '''
        Returns a filtered version of a dataframe. "None" arguments change nothing. Rows with column
    ...
    def export_wip_info(self, df=None):
        ...
        active_part = df['bomitempart'].iloc[0] # grabs the very first item from the filtered df
        ...
        header = {"part_num": active_part,
                  "part_desc": df['partdescription'].iloc[0],
                  "wo_nums": wonums,
                  "total_qty": fgoods['woitemtotal'].sum(),
                  "earliest_date": fgoods['datescheduledfulfillment'].sort_values().iloc[0],
                  "ticket_type": "WIP"
                }
        for i in df.index: 
            self.df.at[i, 'processed'] = True # MARK PROCESSED ITEMS IN THE ORIGINAL DATAFRAME AS PROCESSED=TRUE
        wip_dict = {
            "header": header,
            "fgoods": fgoods,
            "rgoods": rgoods
